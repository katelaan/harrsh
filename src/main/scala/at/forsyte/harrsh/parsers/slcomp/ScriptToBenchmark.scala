...
object ScriptToBenchmark extends HarrshLogging {
  ...
  def apply(s: Script, description: String): Either[SatBenchmark,EntailmentParseResult] = {
      if (s.asserts == 0 || s.asserts.length > 2) {
      ...
      val shs = for{
        a <- s.asserts
      } yield collectAtoms(a.term, env, constsToFvs).head.toSymbolicHeap
      ...
      if(shs.length == 1) {
        Left(SatBenchmark(sid, shs.head, status))
      }
      else {
        assert(shs.length == 2)
        Right(EntailmentParseResult(shs.head, shs(1), sid, status.toBoolean))
      }
