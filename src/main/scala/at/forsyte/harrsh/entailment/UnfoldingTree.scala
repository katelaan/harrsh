...
case class UnfoldingTree private(nodeLabels: Map[NodeId,NodeLabel], root: NodeId, children: Map[NodeId, Seq[NodeId]]) extends HarrshLogging {
  ...
  def interface(diseqs: PureConstraintTracker): EntailmentContext = {
    ...
    EntailmentContext(nodeLabels(root).toPredicateNodeLabel, abstractLeaves map (nodeLabels(_).asInstanceOf[PredicateNodeLabel]), usageInfoMap, diseqs, convertToNormalform = true)
  }
...
object UnfoldingTree extends HarrshLogging {
  ...
  def isInNormalForm(ut: UnfoldingTree): Boolean = {
    ...
    EntailmentContext.noRedundantPlaceholders(ut.nodeLabels.values) && PlaceholderVar.noGapsInPlaceholders(ut.placeholders)
  }
