...
object TrackingAutomata extends SlexLogging {
  ...
  def garbageFreedomAutomaton(numFV : Int) = new BaseReachabilityAutomaton[Boolean](
    ...
    tagComputation = (tags : Seq[Boolean], ti : TrackingInfo, pairs : Set[(FV,FV)], vars : Set[FV]) => !tags.exists(!_) && BaseReachabilityAutomaton.isGarbageFree(ti, pairs, vars, numFV),
  ...
  def acyclicityAutomaton(numFV : Int) = new BaseReachabilityAutomaton[Boolean](
    numFV,
    isFinalPredicate = (_ : BaseReachabilityAutomaton[Boolean], _ : ReachabilityInfo, tag : Boolean) => tag,
    tagComputation = (tags : Seq[Boolean], ti : TrackingInfo, pairs : Set[(FV,FV)], vars : Set[FV]) => !tags.exists(!_) && BaseReachabilityAutomaton.isAcyclic(ti, pairs, vars, numFV),
