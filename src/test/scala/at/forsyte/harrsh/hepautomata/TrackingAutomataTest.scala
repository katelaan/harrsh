...
class TrackingAutomataTest extends HarrshTableTest with TestValues {
  ...
  val transitions = Table(
    ("src", "trg", "sh", "result"),
    // Simple RSHs
    (Seq(), TrackingInfo.fromPair(mkAllVar(1), mkPure()), SymbolicHeap(Seq(ptr(x1, x2))), true),
    (Seq(), TrackingInfo.fromPair(mkAllVar(2), mkPure()), SymbolicHeap(Seq(ptr(x1, x2))), false),
    (Seq(), TrackingInfo.fromPair(mkAllVar(1,2), mkPure((1,2,true))), SymbolicHeap(Seq(ptreq(x1, x2)), Seq(ptr(x1, x2)), Seq()), true),

    // RSHs with some propagation
    (Seq(), TrackingInfo.fromPair(mkAllVar(1,2), mkPure((1,2,false))), SymbolicHeap(Seq(ptr(x1, nil), ptr(x2, nil))), true),
    ...
    (Seq(TrackingInfo.fromPair(mkAllVar(1), mkPure())), TrackingInfo.fromPair(mkAllVar(1,2), mkPure((1,2,false))), SymbolicHeap(Seq(ptr(x2, nil)), Seq(call("dummy", x1))), true),
    ...
    (Seq(TrackingInfo.fromPair(mkAllVar(1), mkPure())), track3.inconsistentState, SymbolicHeap(Seq(ptr(x1, nil)), Seq(call("dummy", x1))), true),
    (Seq(TrackingInfo.fromPair(mkAllVar(1), mkPure()),TrackingInfo.fromPair(mkAllVar(), mkPure((1,2,true)))), track3.inconsistentState, SymbolicHeap(Seq(), Seq(call("sll", x1, x2), call("sll", x2, x3))), false),
    (Seq(TrackingInfo.fromPair(mkAllVar(1), mkPure()),TrackingInfo.fromPair(mkAllVar(), mkPure((1,2,true)))), track3.inconsistentState, SymbolicHeap(Seq(), Seq(call("sll", x2, x1), call("sll", x2, x3))), false),
    (Seq(TrackingInfo.fromPair(mkAllVar(1), mkPure()),TrackingInfo.fromPair(mkAllVar(1), mkPure())), track3.inconsistentState, SymbolicHeap(Seq(ptrneq(x1,x3), ptrneq(x2,x3)), Seq(), Seq(call("sll", x3, x2), call("sll", x3, x1))), true)
  )
