...
class ModelFromTuplesTest extends HarrshTableTest {
  ...
  val testCases = Table(
    ("model", "serialization"),
    (Model.fromTuples( (1, List(2,3), List(Var.defaultFV(1))) ),
      """Stack {
        |  x1 -> 1
        |}
        |Heap {
        |  1 -> 2, 3
        |}""".stripMargin),
    (Model.fromTuples( (1, List(2,3), List(Var.defaultFV(1))), (2, List(4,0,4), List()), (4, List(1,0,3), List(Var.defaultFV(2),Var.defaultFV(3))) ),
      """Stack {
        |  x1 -> 1
        |  x2 -> 4
        |  x3 -> 4
