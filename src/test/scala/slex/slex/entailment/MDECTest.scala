...
import slex.main.main.examples.SymbolicHeapExamples
import slex.seplog.{And, IxEq, IxLSeg, Minus, NullPtr, Plus, PointsTo, PtrNEq, SepCon, SymbolicHeap}
...
class MDECTest extends SlexTest with TableDrivenPropertyChecks {
  ...
  val entailmentPairs = Table(
    ...
    (SymbolicHeap( Seq(PtrNEq("y", "z"), PtrNEq("x", "z")), Seq(PointsTo("x", "y"), PointsTo("y", "z")) ),
    ...
    (SymbolicHeap( Seq(IxEq("i", Plus("j",1))), Seq(IxLSeg("p", "qj", "j"), PointsTo("qj", "q"), IxLSeg("q", NullPtr(), Minus(Minus("n", "j"), 1)))),
     SymbolicHeap( Seq(IxLSeg("p", "q", "i"), IxLSeg("q", NullPtr(), Minus("n", "i")))),
     false),
    ...
    (SymbolicHeap( Seq(IxEq("i", Plus("j",1))), Seq(IxLSeg("p", "qj", "j"), PointsTo("qj", "q"), IxLSeg("q", NullPtr(), Minus(Minus("n", "j"), 1)), PointsTo(NullPtr(), NullPtr()))),
      SymbolicHeap( Seq(IxLSeg("p", "q", "i"), IxLSeg("q", NullPtr(), Minus("n", "i")), PointsTo(NullPtr(), NullPtr()))),
      true),
    ...
    (SymbolicHeap( Seq(IxEq("i", Plus("j",1))),
                   Seq(IxLSeg("p", "qj", "j"), PointsTo("qj", "q"), IxLSeg("q", "r", Minus(Minus("n", "j"), 1))) ),
